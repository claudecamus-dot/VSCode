...
3. **Constraints actually bend design decisions.** A "constraint" that rules nothing out is decoration. Always probe for the most constrained deployment scenario (offline, single-file, specific OS/runtime, air-gapped) — implicit deployment constraints are load-bearing and must be named.
